...
export const CLUB_DOMESTIC_PATCH_METADATA: CompetitionDataMetadata = {
  ...
  notes: [
    ...
    `当前已挂接补录赛事：${ARGENTINE_PRIMERA_DIVISION_PATCH_METADATA.name}、${ARGENTINE_PRIMERA_NACIONAL_PATCH_METADATA.name}、${BRAZIL_SERIE_A_PATCH_METADATA.name}、${BRAZIL_SERIE_B_PATCH_METADATA.name}、${BRAZIL_CUP_PATCH_METADATA.name}、${BRAZIL_SUPER_CUP_PATCH_METADATA.name}、${CAMPEONATO_PAULISTA_PATCH_METADATA.name}、${TORNEIO_RIO_SAO_PAULO_PATCH_METADATA.name}。`
  ]
};
