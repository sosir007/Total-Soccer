...
function getEuropaLeagueEditionUrl(result: SeedEdition) {
  const season = getResultSeason(result.season);
  const startYear = Number(season.slice(0, 4));
  const competitionSlug = startYear <= 2008 ? 'UEFA_Cup' : 'UEFA_Europa_League';
...
function getResultSeason(season: string | null | undefined) {
  if (!season) {
    throw new Error('UEFA Europa League edition season is required to build external URL.');
  }
...
async function main() {
  await runCompetitionSeed({
    ...
    editions: withStandingMode(
      UEFA_EUROPA_LEAGUE_RESULTS.map((result) => ({
        ...result,
        externalUrl: getEuropaLeagueEditionUrl(result)
      }))
    ),
